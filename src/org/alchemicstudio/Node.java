...
public class Node extends BaseObject {
    ...
    /** the name of the currently assigned image pack */
    ...
    public Node(int i, int j, int k, Vector2 vec, int maxSpeedLimit, int minSpeedLimit, int keyId, String type) {
        ...
        mSprite = new Sprite(imagePack, 2, (int)Grid.NODE_DIMENSION, (int)Grid.NODE_DIMENSION);

        if(mType.equals(NODE_TYPE_SPEED_TRAP)) {
            mCurrentImageId = "blue";
        } else if(mType.equals(NODE_TYPE_DEAD)){
            mCurrentImageId = "dead";
        } else if(mType.equals(NODE_TYPE_KEY)) {
            if(mKeyId == Spark.KEY_ID_GREEN) {
                mCurrentImageId = "grey_to_green";
            } else {
                mCurrentImageId = "grey_to_red";
            }
        } else if(mType.equals(NODE_TYPE_GATE)) {
            if(mKeyId == Spark.KEY_ID_GREEN) {
                mCurrentImageId = "green";
            } else {
                mCurrentImageId = "red";
            }
        } else {
            mCurrentImageId = "idle";
        }
    ...
    /**
     * Toggle the image pack id to change
     */
    public void togglePower() {
        if(mNumCurrentConnections > 0) {
            if(mType.equals(NODE_TYPE_STANDARD)) {
                if(mCurrentImageId.equals("idle")) {
                    mCurrentImageId = "white";
                } else {
                    mCurrentImageId = "idle";
                }
            } else if(mType.equals(NODE_TYPE_KEY)) {
                if(mKeyId == Spark.KEY_ID_GREEN) {
                    if(mCurrentImageId.equals("grey_to_green")) {
                        mCurrentImageId = "white_to_green";
                    } else {
                        mCurrentImageId = "grey_to_green";
                    }
                } else {
                    if(mCurrentImageId.equals("grey_to_red")) {
                        mCurrentImageId = "white_to_red";
                    } else {
                        mCurrentImageId = "grey_to_red";
                    }
                }
